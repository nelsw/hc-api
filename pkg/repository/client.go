...
func FindByIds(r value.Request) ([]byte, error) {
	...
	if results, err := db.BatchGetItem(&dynamodb.BatchGetItemInput{
		...
		return nil, err
	} else if err := dynamodbattribute.UnmarshalListOfMaps(results.Responses[r.Table], &r.Result); err != nil {
		return nil, err
	} else {
...
func FindById(r value.Request) ([]byte, error) {
	if out, err := db.GetItem(&dynamodb.GetItemInput{
		TableName: &r.Table,
		Key:       map[string]*dynamodb.AttributeValue{"id": {S: &r.Ids[0]}},
	}); err != nil {
		return nil, err
	} else if err := dynamodbattribute.UnmarshalMap(out.Item, &r.Result); err != nil {
		return nil, err
	} else {
		return json.Marshal(&r.Result)
	}
...
func FindByBrandId(r value.Request) ([]byte, error) {
	...
	if exp, err := expression.NewBuilder().WithFilter(f).Build(); err != nil {
		...
		return nil, err
	} else if err := dynamodbattribute.UnmarshalListOfMaps(out.Items, &r.Result); err != nil {
		return nil, err
	} else {
		return json.Marshal(&r.Result)
	}
...
func FindAll(r value.Request) ([]byte, error) {
...
func Save(r value.Request) ([]byte, error) {
	item, err := dynamodbattribute.MarshalMap(&r.Result)
	if err != nil {
		return nil, err
	}
	...
	if _, err := db.PutItem(&dynamodb.PutItemInput{Item: item, TableName: &r.Table}); err != nil {
		return nil, err
	} else if err := dynamodbattribute.UnmarshalMap(item, &r.Result); err != nil {
		return nil, err
	} else {
		return json.Marshal(&r.Result)
	}
...
func Update(r value.Request) ([]byte, error) {
